...
class DatasetForLlavaInterleave(Dataset):
    def __init__(
        self,
        data_path: str,
        processor,
...
class DatasetForLlava15(Dataset):
    def __init__(
        self,
        data_path: str,
        processor,
    ...
    def __getitem__(self, index: int):
        ...
        out1 = (
            instance_dict_info["chosen_pixel_values"],
            instance_dict_info["chosen_input_ids"],
            instance_dict_info["chosen_labels"],
            instance_dict_info["chosen_attention_masks"],
        )

        out2 = (
            instance_dict_info["rejected_pixel_values"],
            instance_dict_info["rejected_input_ids"],
            instance_dict_info["rejected_labels"],
            instance_dict_info["rejected_attention_masks"],
        )
...
def collate_fn_llava(batch):
    ...
    return (
        ...
        torch.cat(pos_img_neg_ans_pixel_values, dim=0),
        torch.cat(pos_img_neg_ans_input_ids, dim=0),
        torch.cat(pos_img_neg_ans_labels, dim=0),
        torch.cat(pos_img_neg_ans_attention_mask, dim=0),
        torch.cat(neg_img_pos_ans_pixel_values, dim=0),
        torch.cat(neg_img_pos_ans_input_ids, dim=0),
        torch.cat(neg_img_pos_ans_labels, dim=0),
        torch.cat(neg_img_pos_ans_attention_mask, dim=0),
    )
    

def create_data_loader(model_type, data_path, processor, tokenizer, max_seq_len, image_token_index, ignore_token_id, mask_question_tokens):
    if model_type == "llava-interleave":
        dataset = DatasetForLlavaInterleave(data_path, processor, tokenizer, max_seq_len, ignore_token_id, mask_question_tokens)
        collator = collate_fn_llava
    elif model_type == "llava-1.5":
        dataset = DatasetForLlava15(data_path, processor, tokenizer, max_seq_len, ignore_token_id, mask_question_tokens)
        collator = collate_fn_llava
